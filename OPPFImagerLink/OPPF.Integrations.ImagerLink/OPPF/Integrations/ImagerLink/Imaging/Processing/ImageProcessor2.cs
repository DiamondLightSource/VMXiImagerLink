	...
	public class ImageProcessor2 : IImageProcessor
	{
        ...
        public void SetImageInfo(IImageInfo imageInfo)
		{
			...
			try 
			{
                ...
                string filename = global::OPPF.Utilities.FileUtils.GetFilename(imageInfo, _processingInfo, _plateInfo, _plateType);
                ...
                string imageDir = global::OPPF.Utilities.FileUtils.GetDirectory(global::OPPF.Utilities.OPPFConfigXML.GetWebDirectory(), imageInfo, _processingInfo, _robot);
                if (!Directory.Exists(imageDir))
                {
                    Directory.CreateDirectory(imageDir);
                }

                string xmlDir = global::OPPF.Utilities.FileUtils.GetDirectory(global::OPPF.Utilities.OPPFConfigXML.GetXmlDirectory(), imageInfo, _processingInfo, _robot);
                ...
                string rawArchiveDir = global::OPPF.Utilities.OPPFConfigXML.GetArchiveDirectory().Trim();
                if (!"FALSE".Equals(rawArchiveDir, StringComparison.InvariantCultureIgnoreCase))
                {
                    string archiveDir = global::OPPF.Utilities.FileUtils.GetDirectory(rawArchiveDir, imageInfo, _processingInfo, _robot);
                    if (!Directory.Exists(archiveDir))
                    {
                        Directory.CreateDirectory(archiveDir);
                    }
                    // TODO Path.Combine doesn't work as I think?
                    global::OPPF.Utilities.Imaging.ImageUtils.saveAsTiff(imageInfo.Image, Path.Combine(archiveDir, filename));
                }
                ...
                if (ImageType.ExtendedFocus.Equals(imageInfo.ImageType))
                {
                    ...
                    // Special-case for profileid = 3 - teaching
                    if ("3".Equals(_processingInfo.ProfileID) || "4".Equals(_processingInfo.ProfileID))
                    {
                        // TODO Separate archive dir specification from whether or not we are archiving!
                        //string archiveDir = global::OPPF.Utilities.FileUtils.GetDirectory(rawArchiveDir, imageInfo, _processingInfo, _robot);
                        //if (!Directory.Exists(archiveDir))
                        //{
                        //    Directory.CreateDirectory(archiveDir);
                        //}
                        global::OPPF.Utilities.Imaging.ImageUtils.saveAsPng(imageInfo.Image, "C:\\Temp\\Teach\\" + filename);
                    }

                    // TODO: downscale
                    // TODO: set max size
                    Size maxSize = Size.Empty;
                    Image webImage = global::OPPF.Utilities.Imaging.ImageUtils.resizeImage(imageInfo.Image, maxSize);
                    ...
                    global::OPPF.Utilities.Imaging.ImageUtils.saveAsJpeg(webImage, imageDir + filename, 85L);
                    ...
                    // - Mag
                    // - Resolution
                    global::OPPF.XML.ImageInfo xml = new global::OPPF.XML.ImageInfo();
                    ...
                    if (ImageType.ExtendedFocus.Equals(imageInfo.ImageType))
                    {
                        xml.ImageType = global::OPPF.XML.ImageInfoImageType.C;
                    }
                    else
                    {
                        xml.ImageType = global::OPPF.XML.ImageInfoImageType.S;
                    }
                    ...
                    global::OPPF.XML.DoubleSize microns = new global::OPPF.XML.DoubleSize();
                    microns.Height = _captureInfo.Location.Height;
                    microns.Width = _captureInfo.Location.Width;
                    xml.SizeInMicrons = microns;

                    global::OPPF.XML.Size pixels = new global::OPPF.XML.Size();
                    ...
                    xml.Url = global::OPPF.Utilities.OPPFConfigXML.GetImageBaseUrl() + _processingInfo.PlateID + "/" + filename + ".jpg"; // TODO: complete this
        ...
        private DateTime GetDoneFileLastWriteTimeUtc()
        {
            ...
            String donePath = Path.Combine(global::OPPF.Utilities.OPPFConfigXML.GetDoneDirectory(), GetDoneFileName());
